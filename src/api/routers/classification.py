...
async def classify_single(
    request: ClassificationRequest, current_user=Depends(require_role("admin", "doctor", "nurse"))
):
    """
    ...
    try:
        ...
        result = classification_service.classify_incident(
            description=request.description, department=request.department.value if request.department else None
        )
...
async def classify_batch(
    ...
    try:
        ...
        if not any(filename.endswith(ext) for ext in [".csv", ".xlsx", ".xls"]):
            raise HTTPException(status_code=400, detail="Invalid file type. Supported: CSV, Excel (.xlsx, .xls)")
        ...
        if hasattr(file, "size"):
            ...
            if file_size_mb > settings.MAX_UPLOAD_SIZE_MB:
                raise HTTPException(
                    status_code=413, detail=f"File too large. Maximum size: {settings.MAX_UPLOAD_SIZE_MB}MB"
                )
...
async def classify_audio(
    ...
    try:
        ...
        if not any(filename.endswith(ext) for ext in [".mp3", ".wav", ".m4a", ".flac", ".ogg", ".webm"]):
            raise HTTPException(status_code=400, detail="Invalid audio format. Supported: mp3, wav, m4a, flac, ogg")
        ...
        if hasattr(file, "size"):
            ...
            if file_size_mb > settings.AUDIO_MAX_FILE_SIZE_MB:
                raise HTTPException(
                    status_code=413, detail=f"Audio file too large. Maximum: {settings.AUDIO_MAX_FILE_SIZE_MB}MB"
                )
        ...
        try:
            ...
            transcription_result = audio_service.transcribe_and_translate(
                tmp_path, language_code=language, auto_translate=auto_translate
            )
            ...
            if not transcript or len(transcript.strip()) < 10:
                raise HTTPException(
                    status_code=400, detail="Transcription too short or empty. Please provide clearer audio."
                )
            ...
            classification_result = classification_service.classify_incident(
                description=transcript, department=department
            )
