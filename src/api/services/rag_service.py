...
class RAGService:
    ...
    def __init__(self):
        ...
        try:
            self.llm_client = genai.Client(vertexai=True, project=gcp_project, location=gcp_location)

            # Test ChromaDB connection on startup
            try:
                test_client = chromadb.HttpClient(host=self.chromadb_host, port=self.chromadb_port)
    ...
    def _generate_query_embedding(self, query: str) -> List[float]:
        ...
        try:
            ...
            response = self.llm_client.models.embed_content(
                model=self.embedding_model, contents=query, config=types.EmbedContentConfig(**kwargs)
            )
    ...
    def retrieve_policy_context(self, incident_description: str, department: str, n_results: int = 5) -> Dict[str, any]:
        """
        ...
        if not self.rag_available:
            ...
            return {"context": "", "retrieved": False, "error": "RAG service not initialized"}
        ...
        try:
            ...
            if not query_embedding:
                return {"context": "", "retrieved": False, "error": "Failed to generate query embedding"}
            ...
            results = collection.query(
                query_embeddings=[query_embedding], n_results=n_results, where={"department": normalized_dept}
            )

            # Extract and format context
            if results and results["documents"] and len(results["documents"][0]) > 0:
                context_chunks = results["documents"][0]
                context = "\n\n".join([f"[Policy Guideline {i+1}]\n{chunk}" for i, chunk in enumerate(context_chunks)])
                ...
                return {"context": "", "retrieved": False, "error": "No relevant policies found"}

        except Exception as e:
            logger.error(f"Error retrieving policy context: {e}")
            return {"context": "", "retrieved": False, "error": str(e)}

    def augment_prompt_with_context(self, original_prompt: str, context: str) -> str:
        """
