...
MODEL_DIR_LOCAL = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "model"))

# Fallback to parent model directory (for running from repo root)
MODEL_DIR_PARENT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "model"))
...
try:
    from simple_prompt_utils import prompt1_single_incident, prompt2_single_incident, prompt3_single_incident
...
class ClassificationService:
    ...
    def __init__(self):
        ...
        self.department_lookup = {self._normalize_key(dept): dept for dept in settings.VALID_DEPARTMENTS}
    ...
    def classify_incident(self, description: str, department: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            gaps_deviation_bool, gaps_rationale = prompt1_single_incident(
                description, prompt_department, policy_context=policy_context
            )
            ...
            if not gaps_deviation_bool:
                ...
                result["classification_rationale"] = (
                    f"No deviation from Generally Accepted Performance Standards (GAPS). " f"{gaps_rationale}"
                )
            ...
            if not reached_patient_bool:
                ...
                result["classification_rationale"] = (
                    f"Deviation occurred but did not reach the patient. " f"{reached_patient_rationale}"
                )
    ...
    def classify_batch(self, incidents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def process_file(self, file_path: str) -> Tuple[List[Dict[str, Any]], Dict[str, int]]:
        """
    ...
    def _mock_classification(self, description: str, department: str = None) -> Dict[str, Any]:
        """
