...
def setup_containers(connection, configure_docker, project, instance_ip, ssh_user):
    ...
    org_name = config.get("orgName") or pulumi.get_organization()
    images_stack = pulumi.StackReference(f"{org_name}/deploy-images/dev")
    # Get the image tags (these are arrays, so we take the first element)
    api_service_tag = images_stack.get_output("group88-api-service-tags")
    frontend_tag = images_stack.get_output("group88-frontend-react-tags")
    vector_db_cli_tag = images_stack.get_output("group88-vector-db-cli-tags")
    ...
    move_secrets = remote.Command(
        ...
        create="""
            ...
            cat /srv/secrets/gcp-service.json | sudo docker login -u _json_key --password-stdin https://us-central1-docker.pkg.dev
        """,
